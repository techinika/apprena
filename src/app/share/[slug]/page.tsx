...
async function findActivityBySlug(slug: string) {
  try {
    ...
    if (q.exists() && q.data().isPublic) {
      return { id: q.id, ...q.data() } as { id: string; title?: string; userInput?: { goal?: string }; tags?: string[]; isPublic?: boolean; createdAt?: string };
    }
  } catch {
  ...
  try {
    ...
    if (!querySnapshot.empty) {
      ...
      if (docSnap.data().isPublic) {
        return { id: docSnap.id, ...docSnap.data() } as { id: string; title?: string; userInput?: { goal?: string }; tags?: string[]; isPublic?: boolean; createdAt?: string };
      }
    }
  } catch {
    // not found
